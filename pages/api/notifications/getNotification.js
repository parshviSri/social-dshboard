...
const GET_NOTIFICATION = ` query Notifications($profileId : ProfileId!) {
  result: notifications(request: { profileId:$profileId, limit: 10 }) {
    items {
...
`;
const getNotificationByRequest = (ethereumAddress, profileId) => {
  return apolloClient.query({
    query: gql(GET_NOTIFICATION),
    variables: {
      request: {
        ethereumAddress,
         profileId,
...
export const getNotification = async (_address, profileId) => {
  const address = _address;

  const result = await getNotificationByRequest(address, profileId);
